 ...
 */
function findCurrent<T extends Element>(doc: Document, elements: readonly T[]): T | undefined {
 ...
 * scroll frame, so a fast jump can never leave it stale.
 */
export function watchCurrentSection<T extends Element>(
  ...
  const update = (): void => {
    ...
    const next = findCurrent(doc, elements);
